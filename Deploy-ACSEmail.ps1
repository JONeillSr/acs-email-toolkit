...
.PARAMETER AddDomain
    Switch to add a new domain to an existing ACS Email deployment. Creates the
    domain on the Email Communication Service, configures DNS records (if Azure DNS
    is specified), initiates verification, creates MailFrom addresses, and links the
    domain to the Communication Service. Existing linked domains are preserved.
    Use for multi-domain scenarios such as LLCs with multiple DBAs, organizations
    with regional domains, or MSPs managing multiple brands.
    Requires -ResourceGroupName, -EmailServiceName, -CommunicationServiceName,
...
.EXAMPLE
    ...
    Adds a second domain to an existing ACS deployment. Creates the domain,
    DNS records, MailFrom addresses, and links it to the Communication Service
    alongside any previously linked domains. Existing SMTP credentials can
    immediately send from the new domain's MailFrom addresses.
...
.NOTES
    ...
    v2.1.0 - 2026-04-30 - Added -AddDomain mode for multi-domain deployments (LLCs
                          ...
                          when linking additional domains, domain linking command
                          output includes all linked domains for multi-domain setups
...
param(
    ...
    [Parameter(HelpMessage = "Add a new domain to an existing ACS deployment (domain, DNS records, verification, MailFrom addresses, domain linking)")]
    [switch]$AddDomain,
...
function Connect-ACSDomain {
    ...
    .SYNOPSIS
        ...
        Preserves any existing linked domains (for multi-domain deployments).
    #>
    [CmdletBinding(SupportsShouldProcess)]
    param()
    ...
    if ($PSCmdlet.ShouldProcess($CommunicationServiceName, "Link Domain")) {
        # Get existing linked domains to preserve them
        $currentComm = Get-AzCommunicationService -ResourceGroupName $ResourceGroupName -Name $CommunicationServiceName -ErrorAction SilentlyContinue
        $linkedDomains = @()
        if ($currentComm.LinkedDomain) {
            $linkedDomains = @($currentComm.LinkedDomain)
        }

        # Check if already linked
        if ($linkedDomains -contains $domainResourceId) {
            Write-Log "Domain '$domainName' is already linked to Communication Service." -Level INFO
            return
        }

        # Add new domain to existing list
        ...
        try {
            ...
            Write-Log "Domain '$domainName' linked to Communication Service successfully." -Level SUCCESS
            if ($linkedDomains.Count -gt 1) {
                Write-Log "Total linked domains: $($linkedDomains.Count)" -Level INFO
            }
        }
        catch {
            if ($_.Exception.Message -like "*not in a valid state*") {
                ...
                $domainArrayStr = "@('" + ($linkedDomains -join "','") + "')"
                Write-Log "  Update-AzCommunicationService -ResourceGroupName '$ResourceGroupName' -Name '$CommunicationServiceName' -LinkedDomain $domainArrayStr" -Level WARNING
            }
            else {
...
function Invoke-ACSEmailDeployment {
    ...
    try {
        ...
        elseif ($AddDomain) {
            # ============================================================
            # MODE: Add Domain to Existing Deployment
            # ============================================================
            Write-Log "MODE: Add Domain to Existing Deployment" -Level INFO

            Test-Prerequisites

            # Validate required params
            if ([string]::IsNullOrWhiteSpace($CustomDomainName)) {
                Write-Log "-CustomDomainName is required for -AddDomain mode." -Level ERROR
                throw "Missing required parameter: CustomDomainName"
            }

            # Verify the Email Communication Service exists
            ...
            if (-not $existingEmail) {
                Write-Log "Email Communication Service '$EmailServiceName' not found in Resource Group '$ResourceGroupName'." -Level ERROR
                throw "Email Communication Service not found. Run a full deployment first."
            }
            ...
            $existingDomain = Get-AzEmailServiceDomain -ResourceGroupName $ResourceGroupName `
                -EmailServiceName $EmailServiceName -Name $CustomDomainName -ErrorAction SilentlyContinue

            if ($existingDomain) {
                Write-Log "Domain '$CustomDomainName' already exists on this Email Communication Service." -Level INFO
            }
            else {
                # Create the domain with retry
                Write-Log "Waiting 5 seconds before domain creation..." -Level INFO
                Start-Sleep -Seconds 5
                New-ACSEmailDomain
            }

            # Create MailFrom addresses
            ...
            if ($domainStatus) {
                $allVerified = (
                    $domainStatus.DomainVerificationStatusDomain -eq "Verified" -and
                    $domainStatus.DomainVerificationStatusSpf -eq "Verified" -and
                    $domainStatus.DomainVerificationStatusDkim -eq "Verified" -and
                    $domainStatus.DomainVerificationStatusDkim2 -eq "Verified"
                )
            }

            if ($allVerified) {
                Write-Log "Domain '$CustomDomainName' is fully verified." -Level SUCCESS

                # Link domain - need to ADD to existing linked domains, not replace
                Write-Log "Linking domain '$CustomDomainName' to Communication Service..." -Level INFO
                $subscriptionId = (Get-AzContext).Subscription.Id

                $newDomainResourceId = "/subscriptions/$subscriptionId/resourceGroups/$ResourceGroupName/providers/Microsoft.Communication/emailServices/$EmailServiceName/domains/$CustomDomainName"

                # Get existing linked domains
                $currentComm = Get-AzCommunicationService -ResourceGroupName $ResourceGroupName -Name $CommunicationServiceName
                $linkedDomains = @()
                if ($currentComm.LinkedDomain) {
                    $linkedDomains = @($currentComm.LinkedDomain)
                }

                # Add new domain if not already linked
                if ($linkedDomains -contains $newDomainResourceId) {
                    Write-Log "Domain '$CustomDomainName' is already linked." -Level INFO
                }
                else {
                    $linkedDomains += $newDomainResourceId
                    try {
                        Update-AzCommunicationService `
                            -ResourceGroupName $ResourceGroupName `
                            -Name $CommunicationServiceName `
                        ...
                        Write-Log "Domain '$CustomDomainName' linked successfully." -Level SUCCESS
                    }
                    catch {
                        Write-Log "Failed to link domain: $($_.Exception.Message)" -Level WARNING
                        Write-Log "Link manually after verification completes." -Level WARNING
                    }
                }
                ...
                Write-Log " " -Level INFO
                Write-Log "============================================================" -Level SUCCESS
                Write-Log "  DOMAIN '$($CustomDomainName.ToUpper())' ADDED SUCCESSFULLY" -Level SUCCESS
                Write-Log "============================================================" -Level SUCCESS
                Write-Log " " -Level INFO
                Write-Log "DOMAIN DETAILS:" -Level INFO
                Write-Log "  Domain:         $CustomDomainName" -Level INFO
                ...
                Write-Log " " -Level INFO
                Write-Log "Existing SMTP credentials can now send from this domain." -Level INFO
                Write-Log "Set the From address to any MailFrom above when sending." -Level INFO
                Write-Log "To create a dedicated SMTP endpoint for this domain, use -AddSmtpEndpoint." -Level INFO
                Write-Log "============================================================" -Level SUCCESS
            }
            else {
                # Not verified yet - provide instructions
                $subscriptionId = (Get-AzContext).Subscription.Id
                $portalUrl = "https://portal.azure.com/#@/resource/subscriptions/$subscriptionId/resourceGroups/$ResourceGroupName/providers/Microsoft.Communication/emailServices/$EmailServiceName/provision"

                Write-Log " " -Level INFO
                Write-Log "============================================================" -Level SUCCESS
                Write-Log "  DOMAIN '$($CustomDomainName.ToUpper())' ADDED - VERIFICATION PENDING" -Level SUCCESS
                Write-Log "============================================================" -Level SUCCESS
                Write-Log " " -Level INFO
                Write-Log "Domain and DNS records have been configured." -Level SUCCESS
                ...
                Write-Log " " -Level INFO
                Write-Log "  Step 2: After all four show 'Verified', link the domain:" -Level INFO
                Write-Log " " -Level INFO

                # Build the link command with existing domains preserved
                $currentComm = Get-AzCommunicationService -ResourceGroupName $ResourceGroupName -Name $CommunicationServiceName
                $linkedDomains = @()
                if ($currentComm.LinkedDomain) {
                    $linkedDomains = @($currentComm.LinkedDomain)
                ...
                Write-Log "          Update-AzCommunicationService ``" -Level INFO
                Write-Log "              -ResourceGroupName '$ResourceGroupName' ``" -Level INFO
                Write-Log "              -Name '$CommunicationServiceName' ``" -Level INFO
                Write-Log "              -LinkedDomain $domainArrayStr" -Level INFO
                Write-Log " " -Level INFO
                Write-Log "  Or re-run: .\Deploy-ACSEmail.ps1 -AddDomain ``" -Level INFO
                Write-Log "              -ResourceGroupName '$ResourceGroupName' ``" -Level INFO
